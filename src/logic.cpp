...
const char* LOG_FILE_NAME = "/tmp/example.log";
}

Logic::Logic()
{
    log_file.open(LOG_FILE_NAME, std::ofstream::trunc);
    if (log_file.fail()) {
        std::cerr << "[ERROR]: Can't open " << LOG_FILE_NAME
                  << ", is the directory writable?" << std::endl;
    }
}
...
void Logic::add_number(const int16_t new_element)
{
    /*
...
void Logic::check_necessity_and_log()
{
    ...
    if (m_median_elem > 5 * (m_current_row_average)) { // 5*average(x[max_pos-5000]...x[max_pos+5000])
        ...
        float log_data_comparison = (m_median_elem ? static_cast<float>(m_median_elem) / static_cast<float>(m_current_row_average) : 0);
        ...
        std::stringstream ss;
        ss << "Local maximum is found! Position: " << log_data_position_since_start
